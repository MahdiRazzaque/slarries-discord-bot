  ...
  async execute(interaction, client) {
    ...
    switch (choices) {
        ...
        case "frequent-questions": {
          ...
          const whyWitherCloakSword = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("Why wither cloak sword?").setDescription("> The wither cloak sword ability (creeper viel) does not consume mana to toggle but does have a 1s cooldown \n \n> When the ability is active, 6 invisible Charged Creepers surround the player. \n \n > The ability nullifies any damage taken at the cost of 20% of your mana (10% with ultimate wise V) \n \n> This allows you to survive the boss even the beacons of the voidgloom seraph.").setColor("BLUE")]})
          const witherCloakSwordImageSend = await client.channels.cache.get(gChannel.id).send({ files: [witherCloakSwordImage]})
          const whyGoblinArmour = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("Why goblin armour?").setDescription("> Goblin's full set bonus leaves only the player with 100 max mana. \n \n> Since the creeper viel ability takes 20% of your mana per hit nullified (10% with ultimate wise V), it is quicker to regenerate 10 mana rather than for example 1k mana in storm.")]})
          const goblinArmourImageSend = await client.channels.cache.get(gChannel.id).send({ files: [goblinArmourImage]})
          const whatToDoWhenMini = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("What to do if a miniboss spawns.").setDescription('> Slayer minibosses have a chance to spawn after the player kills the corresponding slayer mob during a slayer quest of Tier III or higher. \n \n > If you are unable to kill the miniboss yourself "/msg [carrier IGN] mini" and the carrier will come to kill it \n \n> As long as you hit it once it will give you the experience.').setColor("BLUE")]})
          const whatToDoWhenBoss = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("What to do when the boss spawns.").setDescription("> When the boss spawns \"/msg [carrier IGN] boss\" and the carrier will come to kill it . \n \n > Follow the instructions in <#917063326587031603> to make sure you don't die. \n \n> As long as you hit it once it will give you the experience.").setColor("BLUE")]})
          const whatToDoIfDeath = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("What to do if you or the carrier die.").setDescription("> If you or the carrier dies, you can either try again or recieve a full refund.").setColor("BLUE")]})
        }
        break;
      }
