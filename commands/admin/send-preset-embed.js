  ...
  async execute(interaction, client) {
    ...
    switch (choices) {
        ...
        case "slayer-prices": {
          ...
          const voidgloomSeraph = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("Voidgloom Seraph").setDescription("> Tier 3 - 400K \n> Tear 4 - 1.4m").setColor("BLUE")]})
        ...
        case "slayer-survival": {
          ...
          const suriveVoidgloomSeraph = await client.channels.cache.get(gChannel.id).send({embeds: [new MessageEmbed().setTitle("Voidgloom Seraph").setDescription("> Try to spawn the boss at the top of the void sepulture. \n \n > Once the boss has spawned telport to the top of the obsidian structure (the carrier will show you where if you don't know). \n \n > Minimum items needed to survive boss: \n> \`•\` Mithril coat \n> \`•\` Wand of mending (preferably restoration) \n> \`•\` Zombie sword (preferably florid) \n> \`•\` Preferably bonzo mask or reaper mask \n \n > **IMPORTANT:** Make sure you look at flying heads during final phase. \n \n > Enjoy the show!").setColor("BLUE")]})
        ...
        case "frequent-questions": {
          ...
          const whatToDoIfDeath = new MessageEmbed().setTitle("What to do if you or the carrier die.").setDescription("> If the carrier dies, you can either try again or recieve a full refund. \n\n > If you die, refunds can be made if the death wasn't your fault.").setColor("BLUE")
