...
client.distube
    ...
    .on("searchNoResult", message => message.channel.send({embeds: [new MessageEmbed().setColour("RED").setDescription(`❌ | No result found!`)]}))
    .on("finish", queue => queue.textChannel.send({embeds: [new MessageEmbed().setColor("RED").setDescription("🏃‍♂️ Queue finished, leaving the channel.")]}))